 ...
 *
 * reference_number (DB column: leases.reference_number):
 *   - INTERNAL identifier. Generated by LeaseReferenceService.
 *   - Format: NRB-CH-MAJ-484A-001-2026-052 (zone code + source + type + unit + year + sequence)
 *   - Used for: internal tracking, QR codes, system URLs, API lookups, logs
 *   - Generated: automatically on lease creation (LeaseObserver / CreateLease)
 *   - Mutable: NO — never changes after generation
 *   - Visible to: Chabrin staff, managers, field officers (admin panel)
 *
 * lease_reference_number (DB column: leases.lease_reference_number):
 *   - FORMAL contract identifier. Set by the Chabrin legal/admin team.
 *   - Format: Free-form legal numbering (e.g. "CHAB/2026/NRB/001")
 *   - Used for: printed on the physical/digital lease document, court filings,
 *               stamp duty registration, legal correspondence
 *   - Generated: manually entered by admin, OR auto-populated from the
 *                landlord's own numbering when source='landlord_provided'
 *   - Mutable: YES — can be updated before lease is ACTIVE
 *   - Visible to: tenant (appears on their signed PDF), lawyer, landlord
 *
 * Rule: reference_number tracks the lease in the system.
 *       lease_reference_number is what the courts and KRA see.
 */
class LeaseReferenceService
{
     ...
     *
     * Format: [ZONE]-[SOURCE]-[TYPE]-[UNIT_CODE]-[YEAR]-[SEQ]
     * Example: NRB-CH-MAJ-484A-001-2026-052
     *
     ...
     * @param int|null $zoneId Optional zone ID; prepends zone code when provided
     *
     * @throws Exception If source or lease type is invalid
     *
     * @return string The generated reference number
     */
    public static function generate(string $source, string $leaseType, string $unitCode, ?int $year = null, ?int $zoneId = null): string
    {
        // Validate source
    ...
    private static function zoneCodeForId(int $zoneId): ?string
    {
        $zone = Zone::find($zoneId);

        return $zone?->code ? strtoupper(substr($zone->code, 0, 5)) : null;
    }
    ...
    public static function generateForLease(Lease $lease, ?int $year = null): string
    {
        ...
        return self::generate($lease->source, $lease->lease_type, $unitCode, $year, $lease->zone_id);
    }
